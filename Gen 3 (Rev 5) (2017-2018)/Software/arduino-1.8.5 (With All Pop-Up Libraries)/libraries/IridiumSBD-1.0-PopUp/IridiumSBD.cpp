...
void IridiumSBD::dbg(FlashString msg)
{
   ...
   if (this->pDiagsStream)
      	{pDiagsStream->print(msg);	//pDiagsStream->flush();
}  
#endif
}
...
#if ISBD_DIAGS
   if (this->pDiagsStream)
      		{pDiagsStream->print(n);	//pDiagsStream->flush();
} 
#endif
}
...
#if ISBD_DIAGS
   if (this->pDiagsStream)
      		{pDiagsStream->print(c);	//pDiagsStream->flush();
}
#endif
}
...
void IridiumSBD::console(FlashString msg)
{
   if (this->pConsoleStream)
      	{pConsoleStream->print(msg);//pConsoleStream->flush();
}}
...
//pConsoleStream->flush();
}}

void IridiumSBD::console(uint16_t n)
{
   if (this->pConsoleStream)
      		{pConsoleStream->print(n);//pConsoleStream->flush();
...
console("\r\n");                                           //This line added by Dan Langis Dec 2016 to process 'OK\r\n' at end of iridium response
if (!waitForATResponse())								   //This line added by Dan Langis Dec 2016 to process 'OK\r\n' at end of iridium response
return cancelled() ? ISBD_CANCELLED : ISBD_PROTOCOL_ERROR;
