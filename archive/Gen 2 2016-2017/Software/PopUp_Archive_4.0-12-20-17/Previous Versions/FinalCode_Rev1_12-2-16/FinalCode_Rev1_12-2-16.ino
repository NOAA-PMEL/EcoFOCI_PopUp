...
DateTime unitStart =  DateTime(16,12,6,13,12,0);             //Date and Time for first sample: 				DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
DateTime releaseTime =  DateTime(16,12,6,13,21,0);           //Date and Time for actual release of burn wire: 		DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
DateTime sendDataDate = DateTime(16,12,6,13,24,0);           //Date and Time to attempt data transmission twice a day: 	DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
DateTime iceFreeDate = DateTime(16,12,6,13,24,0);            //Date and Time to attempt data transmission every hour: 	DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
...
void setAlarmNextSendAttempt(){
  ...
  if(whichFile=='z'){                                   //if All the data has finished sending, go into sleep mode, set by designated sleep interval in seconds (stored in filePosition variable)
    ...
    alarmtime=now.unixtime()+(unsigned long)3600;       //Add 1 hour to the current time 
    Serial.print(F("Alarm Interval="));
    Serial.print(F("3600"));
  }
  if(whichFile!='x'){
