...
#define SN 2007                      //INPUT POP-UP S/N as SN = ####

//INPUT for Topside Temperature Probe (TTP)
float At = 0.00119770835;  //INPUT Steinhart-Hart Coeficient A for probes from unit of interest
float Bt = 0.000228621235;  //INPUT Steinhart-Hart Coeficient B for probes from unit of interest
float Ct = 0.000000114542981;  //INPUT Steinhart-Hart Coeficient C for probes from unit of interest

//INPUT for Sea Surface Temperature (SST) Probe
float As = 0.00110229216;  //INPUT Steinhart-Hart Coeficient A for probes from unit of interest
float Bs = 0.000237313424;   //INPUT Steinhart-Hart Coeficient B for probes from unit of interest
float Cs = 0.00000013037911;  //INPUT Steinhart-Hart Coeficient C for probes from unit of interest
...
#define kellerADDR 0x40                         //Define pressure sensor control register
...
void displaySensor(char menuInput) {                  //function to display measurement from sensors once every 1/4 second, menuInput defines which sensor to display
  ...
  while (waitingforInput2 == true) {                  //until the user Enters 'Q' to stop...
    ...
    delay(0);                                      //take 4 sensor readings per second, once every 250 milliseconds
  }
...
void TestSSTResponse() {
  ...
  int tempRefVal = readTempRef();
  float tempRefCalc = (tempRefVal / 16384.0 * 3.0 / 4.0) * 39.872;
  Serial.print(tempRefCalc, 2);
  Serial.print(","); 
  Serial.print(millis());
  Serial.print(","); 
  Serial.println(SSTCalc, 2);                                                                                   //the '2' after SSTCalc, '2' designates taking temp measurement to the nearest 100th of a degree
}
...
void TestTTPResponse() {
  ...
  Serial.print(millis());
  Serial.print(","); 
  Serial.println(TTPCalc, 2);                                                                                   //the '2' after TTPCalc, '2' designates taking temp measurement to the nearest 100th of a degree
}
...
boolean setupCamera() {                                                     //function to setup camera
  ...
  uint8_t INITIAL_COMMAND[6] = {0xAA, 0x01, 0x00, 0x07, 0x00, 0x07};        //6 byte command to configure camera settings (resolution 07 = 640x480 + format 07 = jpeg)
...
int16_t testSST() {                      //function to read temperature ADC for SST thermistor 
  digitalWrite(TEMP_SWITCH_1, HIGH);     //send signal to switch to top thermistor reading
  digitalWrite(TEMP_SWITCH_2, HIGH);     //send signal to switch to top thermistor reading
  delay(333);                            //sample ADC 4 times per second (4Hz or once every 250 milliseconds)
  int16_t measurement = readTempADC();   //read the ADC (Same for thermistor or reference)
  digitalWrite(TEMP_SWITCH_1, LOW);      //send signal to switch back to reference resistor 1
  digitalWrite(TEMP_SWITCH_2, LOW);      //send signal to switch back to reference resistor 2
  return measurement;                    //return the measured value from the ADC
...
int16_t readTempRef() {                  //function to read temperature ADC for temp ref
  delay(0);                           //delay to let voltage stabilize
  int16_t measurement = readTempADC();   //read the ADC (Same for thermistor or reference)
  return measurement;                    //return the measured value from the ADC
}
