...
DateTime wakeupTime;
DateTime alarmtime;
DateTime releaseTime =  DateTime(16,9,16,13,0,0);                  //DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
DateTime sendDataDate = DateTime(16,9,16,13,30,0);                   //DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
DateTime iceFreeDate = DateTime(16,9,16,13,30,0);                   //DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
...
char selectMode(){
  ...
  if(wakeupTime.unixtime()<(releaseTime.unixtime()-1800)){                                                              //if more than 6 hours from release time, just take bottom sample
    mode=1;
  }
  ...
  else if((wakeupTime.unixtime()>=(releaseTime.unixtime()-1800))&&(wakeupTime.unixtime()<=(releaseTime.unixtime()))){   //if 6 hours or less from release time,go to WaitForProfile mode
    mode=2;
  }  
...
void sampleData(char mode){
  ...
  if(mode==1||mode==2){                                    //if at least 6 hours 1 minute from releaseWakeDate and unit is on bottom, set alarm for 6 hours from now
    dataFile = SD.open("bottest.txt", FILE_WRITE);          //open the file with write permissions
  }
  else if(mode==3||mode==4){
    dataFile = SD.open("icetest.txt", FILE_WRITE);          //open the file with write permissions
  }
  else if(mode==5){
    dataFile = SD.open("protest4.txt", FILE_WRITE);          //open the file with write permissions      
  }
...
void setAlarm(char mode){
  if(mode==1){                                                                      
    if(wakeupTime.unixtime()<(releaseTime.unixtime()-43260)){              //if more than 12 hours 1 minute from releaseTime set alarm for 6 hours from now
      ...
      alarmtime = wakeupTime.unixtime()+300;                                //set alarm for 6 hours from wakeupTime (minus any spare seconds)
    }      
...
void waitForProfile(){
  Serial.println(F("waiting for profile"));
  unsigned int pressureAverageLong = averagePressureLong();
  Serial.print(F("pressureAverageLong="));
  Serial.println(pressureAverageLong);
  ...
  while(pressureAverageShort>(pressureAverageLong-500)){          //while average of last 3 depth readings is at least .95 of original established depth
    now = RTC.now();                                     
    if(now.unixtime()>(releaseTime.unixtime()+21540)){             //if 5hr59m hours past expected release time, shut the unit down (alarm already set for releaseTime+7 hours)
      Serial.println(F("Passed Release time:Shutting Down"));    
      shutdownUnit();
    }
    delay(1000);
    pressureAverageShort=averagePressureShort();                  //update the short running depth average every 2ish seconds
    Serial.print(F("pressureAverageShort="));
    Serial.println(pressureAverageShort);
  }
  Serial.println(F("Pressure Tripped, switching to profile mode"));    
}
...
void collectProfile(){
  ...
  long endProfileTime=millis()+10000;  //10,000 ms = 10 sec
