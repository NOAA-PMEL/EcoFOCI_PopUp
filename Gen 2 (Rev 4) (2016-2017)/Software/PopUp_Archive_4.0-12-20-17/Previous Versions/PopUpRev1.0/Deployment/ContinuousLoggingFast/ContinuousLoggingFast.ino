...
void loop(){  
  unsigned long timeStamp=millis()+500;   
  sampleData(5);
...
void sampleData(char mode){
  ...
  if(mode==5){
    dataFile = SD.open("pro.txt", FILE_WRITE);       //open the file with write permissions      
  }
...
void setAlarm(char mode){
  ...
  if(mode==1){                                                                      
    ...
    alarmtime = wakeupTime.unixtime()+3600;   //set alarm for 6 hours from wakeupTime (minus any spare seconds)
  }
  else if(mode==6){
    DateTime now = RTC.now();
    alarmtime = now.unixtime()+3600;   //set alarm for 6 hours from wakeupTime (minus any spare seconds)
  }
  else{
