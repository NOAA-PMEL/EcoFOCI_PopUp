...
DateTime wakeupTime;
DateTime alarmtime;
DateTime unitStart =  DateTime(16,9,16,9,41,0);                    //DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
DateTime releaseTime =  DateTime(16,9,16,9,46,0);                  //DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
DateTime sendDataDate = DateTime(16,9,16,9,50,0);                   //DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
DateTime iceFreeDate = DateTime(16,9,16,9,50,0);                   //DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
...
void sampleData(char mode){
  ...
  if(mode==1||mode==2){                                    //if at least 6 hours 1 minute from releaseWakeDate and unit is on bottom, set alarm for 6 hours from now
    dataFile = SD.open("botben.txt", FILE_WRITE);          //open the file with write permissions
  }
  else if(mode==3||mode==4){
    dataFile = SD.open("iceben.txt", FILE_WRITE);          //open the file with write permissions
  }
  else if(mode==5){
    dataFile = SD.open("proben.txt", FILE_WRITE);          //open the file with write permissions      
  }
...
void setAlarm(char mode){
  if(mode==1){                                                                      
    ...
    //if(wakeupTime.unixtime()<(releaseTime.unixtime()-600)){               //if more than 10 minutes from releaseTime set alarm for 5 min from now
    if(wakeupTime.unixtime()<(releaseTime.unixtime()-130)){                 //if more than 2 minutes, 10s from releaseTime set alarm for 1 min from now
      ...
      alarmtime = wakeupTime.unixtime()+60;                                 //set alarm for 1 minutes from wakeupTime 
    }      
    ...
    alarmtime = releaseTime.unixtime()+300;                               //set alarm for 5 min after releaseTime 
  }
  else if(mode==3||mode==4){
...
void collectProfile(){
  ...
  long endProfileTime=millis()+60000;  //10,000 ms = 10 sec
