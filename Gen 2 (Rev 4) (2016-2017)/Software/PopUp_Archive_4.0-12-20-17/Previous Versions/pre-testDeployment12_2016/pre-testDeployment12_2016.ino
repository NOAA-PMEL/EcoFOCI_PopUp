...
DateTime unitStart =  DateTime(16,12,8,11,48,0);              //Date and Time for first sample: 				DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
DateTime releaseTime =  DateTime(16,12,8,12,00,0);           //Date and Time for actual release of burn wire: 		DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
DateTime sendDataDate = DateTime(16,12,8,12,15,0);           //Date and Time to attempt data transmission twice a day: 	DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
DateTime iceFreeDate = DateTime(16,12,8,12,15,0);            //Date and Time to attempt data transmission every hour: 	DateTime(YEAR,MON,DAY,HOUR,MIN,SEC);
...
#define PROFILE_LENGTH_SECONDS 10			    //Length of profile in seconds				(nominal PROFILE_LENGTH_SECONDS 60 = 60 seconds)
...
char selectModeandSetAlarm(){
  ...
  Serial.print(F("wakeUpTime:"));
  Serial.println(wakeupTime.unixtime());
  Serial.print(F("alarmTime:"));
...
void sampleData(char modeSelected){
  ...
  if (dataFile) {                                           //if file is available, write to it:
    ...
    if(modeSelected==3){				    //If in profiling mode, then we want a new timestamp for each sample
      ...
      dataFile.print(wakeupTime.unixtime());            //If not in profiling mode, use the timestamp for when the unit woke up
    }
    dataFile.print(F(","));                                 //,
...
//Serial.println(gpsDelaySearchMillis);
...
//for(int q=0;q<=CHARS_PER_SBDMESSAGE;q++){
//Serial.print(q);
//Serial.print(F(","));
//Serial.print(iridiumSendBuffer[q]);
//Serial.print(F(","));
//Serial.println(int(iridiumSendBuffer[q]));
//}    
...
Serial.println(filePosition);  
  ...
  SD.remove("summary.txt");                                                             //Delete old file and any contents
...
Serial.println(filePosition);  
  SPI.setDataMode(SPI_MODE0);                                                           //Set SPI Data Mode to 0 for sd card
  SD.remove("filepos.txt");                                                             //Delete old file and any contents
...
void setAlarmNextSendAttempt(){
  ...
  if(whichFile=='z'){                                   //if All the data has finished sending, go into sleep mode, set by designated sleep interval in seconds (stored in filePosition variable)
    if(filePosition==0){          
      filePosition=86400;                               //if a designated sleep interval is not set from previous SD write or received message, then default to +1 day
    }
